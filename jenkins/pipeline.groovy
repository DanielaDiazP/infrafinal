pipeline {

    environment {
        BASE_GIT_URL = 'https://github.com/DanielaDiazP'
        APP_REPO_URL = "${env.BASE_GIT_URL}/${nombre_repositorio}.git"
        INFRA_REPO_URL = "${env.BASE_GIT_URL}/infrafinal.git"
        DOCKER_IMAGE = "danieladiazp/${nombre_repositorio}"
        DEPLOY_FOLDER = "deployfinal/kubernetes/${nombre_repositorio}"
    }

    agent any
    stages {
        stage("Checkout app-code") {
            steps {
               dir('appfinal') {
                    git url:"${env.APP_REPO_URL}" , branch: "${version}"
                } 
            }
        }
         
         stage("Checkout deploy-code") {
            steps {
               dir('deployfinal') {
                    git url:"${env.INFRA_REPO_URL}" , branch: "master"
                } 
            }
        }
        
         stage("Build image") {
            steps {
                dir('appfinal') {
                    script {
                        dockerImage = docker.build("${env.DOCKER_IMAGE}:${tag}")
                    }
                }
            }
        }
        
        stage("Push image") {
            steps {
                script {
                    docker.withRegistry('', 'danieladocker') {
                    dockerImage.push()
                    }
                }
            }
        }
        
        stage('Deploy') {
            steps{
                sh "sed -i 's:REPLICA:${replica}:g' ${DEPLOY_FOLDER}/deployment.yaml"
                sh "sed -i 's:DOCKER_IMAGE:${env.DOCKER_IMAGE}:g' ${DEPLOY_FOLDER}/deployment.yaml"
                sh "sed -i 's:TAG:${tag}:g' ${DEPLOY_FOLDER}/deployment.yaml"
                
                step([$class: 'KubernetesEngineBuilder', 
                        projectId: "nice-root-288300",
                        clusterName: "cluster-daniela",
                        zone: "us-central1-a",
                        manifestPattern: "${DEPLOY_FOLDER}/",
                        credentialsId: "seminario",
                        verifyDeployments: true])
            }
        }
    }
}